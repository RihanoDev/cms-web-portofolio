...
export default function ArticlesEditor() {
  ...
  const [articleViews, setArticleViews] = useState<Record<string, number>>({}); // assignment is not useless, used for view counts
  useEffect(() => { 
    // Load articles
  ...
  const addNewArticle = () => {
    ...
    setEditingIndex(articlesArray.length);
  };
  ...
  const saveArticles = async () => {
    ...
    try {
      ...
      await ContentStore.saveArticles(articlesArray);
      setShowSuccess(true);
