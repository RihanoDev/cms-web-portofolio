...
export const trackContentView = async (params: TrackViewParams): Promise<boolean> => {
  try {
    ...
    await api.post('/api/v1/views/track', params);
...
export const getContentViews = async (
  ...
  try {
    ...
    const response = await api.get(`/api/v1/views/count?contentId=${contentId}&contentType=${contentType}`);
...
export const getViewsAnalytics = async (
  ...
  try {
    const url = `/api/v1/views/analytics?contentId=${contentId}&contentType=${contentType}&period=${period}&limit=${limit}`;
